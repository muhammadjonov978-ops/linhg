...
interface SpeakingWordExercise {
  id: string;
  title: string;
  words: string[];
}

interface SpeakingSentenceExercise {
  id: string;
  title: string;
  sentences: string[];
}

type SpeakingExercise = SpeakingWordExercise | SpeakingSentenceExercise;
...
export default function SpeakingSection({ exercises, langId, levelId: _levelId, onComplete }: SpeakingSectionProps) {
  ...
  const [results, setResults] = useState<Record<number, { spoken: string; target: string; accuracy: number; passed: boolean }>>({});
  ...
  return (
    <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="w-20 h-2 bg-base-200 rounded-full overflow-hidden">
            <div className="h-full bg-error rounded-full transition-all duration-500" style={{ width: `${(progress / total) * 100}%` }} />
          </div>
        </div>
      </div>
      ...
      {!showResults ? (
        <>
          <div className="text-center text-sm opacity-60">{currentItemIndex + 1} / {items.length}</div>
            ...
            <div className="card-body items-center text-center py-8">
              <div className="mb-6">
                ...
                <div className="text-2xl font-bold p-4 bg-base-200 rounded-xl">
                  &ldquo;{currentItem}&rdquo;
                </div>
              </div>

              <div className="flex gap-4 mb-6">
                <button onClick={playCurrentItem} disabled={isListening || isPlaying} className="btn btn-outline btn-lg btn-circle" title="Eshitish">
                  <Volume2 className={`w-6 h-6 ${isPlaying ? 'animate-pulse' : ''}`} />
                </button>
                <button
                  onClick={isListening ? stopListening : startListening}
                  disabled={isPlaying || isProcessing}
                  className={`btn btn-lg btn-circle ${isListening ? 'btn-error animate-pulse' : 'btn-primary'}`}
              ...
              <div className="w-full max-w-md">
                ...
                <button onClick={handleCheck} disabled={!recognitionResult.trim() || isListening || isProcessing} className="btn btn-primary w-full">
                  {isProcessing ? <>Tekshirilmoqda...</> : <><CheckCircle className="w-4 h-4" /> Talaffuzni tekshirish</>}
                </button>
              </div>
              ...
              {Object.entries(results).map(([idx, res]) => (
                <div key={idx} className={`flex items-center gap-2 p-2 rounded-lg text-sm ${res.passed ? 'bg-success/10' : 'bg-error/10'}`}>
                  ...
                  <span className="flex-1">
                    <span className={res.passed ? 'text-success' : 'text-error'}>&ldquo;{res.spoken}&rdquo; </span>
                    <span className="text-xs opacity-50 ml-2">({res.accuracy}%)</span>
                  </span>
                  {!res.passed && <span className="text-xs opacity-60">→ &ldquo;{res.target}&rdquo;</span>}
                </div>
              ))}
            </div>
          )}
          ...
          <div className={`card ${score >= 80 ? 'bg-success/10 border-success' : score >= 50 ? 'bg-warning/10 border-warning' : 'bg-error/10 border-error'} border shadow-sm`}>
            <div className="card-body text-center py-8">
              ...
              <p className="text-sm opacity-70">
                {score >= 80 ? '🎉 Ajoyib talaffuz!' : score >= 50 ? '💪 Yaxshi, yana mashq qiling!' : '🔄 Qayta urinib ko\'ring!'}
              </p>
            </div>
          </div>
