...
export default function MistakesReview() {
  ...
  // Collect mistakes from progress (lessons with score < 80 but > 0)
  useEffect(() => {
    const collected = [];
    Object.entries(state.progress).forEach(([key, prog]) => {
      const parts = key.split('-');
      const langId = parts[0];
      const lessonNum = key.match(/lesson-(\d+)/)?.[1];
      if (prog.score > 0 && prog.score < 80) {
        collected.push({
          id: key,
          langId,
          levelId: lessonNum ? `lesson-${lessonNum}` : 'unknown',
          skill: 'reading',
          score: prog.score,
          timestamp: prog.timestamp,
          lang: MISTAKE_LANGUAGES.find(l => l.id === langId),
          level: lessonNum ? MISTAKE_LEVELS.find(l => l.id === 'beginner') : null,
        });
      }
    });
            ...
            {!isFlipped ? (
              <div className="text-center">
                ...
                <p className="text-xs opacity-50 mb-2">
                  {currentMistake.lang?.flag} {currentMistake.lang?.name} • {currentMistake.level?.icon} {currentMistake.level?.name}
                </p>
